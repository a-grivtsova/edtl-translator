package su.nsk.iae.edtl.generator.verifier.util
...
class LiteralUtils {
	...
	def static long parseTime(String literal) {
		var rest = literal.toLowerCase.substring(0, literal.length)
		var int index;
		var long millis = 0;
	...
	def static getPortIndex (String location) {
		return location.indexOf(".") == -1
			? location.substring(3)
			: location.substring(3, location.indexOf("."))
	}
	
	def static getBitIndex (String location) {
		var index = location.indexOf(".") + 1
		return index > 0 && index < location.length + 2
			? location.substring(index, index+1)
			: 0
	}
}